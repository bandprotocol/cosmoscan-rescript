...
describe("Expect GraphQLParser to work correctly", () => {
  ...
  test("buffer", () => expect({
    ...
    json->buffer
  })->toEqual("0x0235b618ab0f0e9f48b1af32f56b78d955c432279893714737a937035024b83c58"->JsBuffer.fromHex))
  ...
  test("timestampWithDefault", () => expect({
    let json = `"2023-05-11T06:24:46.511Z"`->Js.Json.parseExn
  ...
  test("timestampWithDefault Default case", () => expect(
    None->timestampWithDefault->MomentRe.Moment.toUnix
  )->toEqual(MomentRe.momentNow()->MomentRe.Moment.toUnix))

  test("stringExn", () => expect({Some("test")->stringExn})->toEqual("test"))

  test("optionBuffer", () => expect({
    let json = `"0x0235b618ab0f0e9f48b1af32f56b78d955c432279893714737a937035024b83c58"`->Js.Json.parseExn

    Some(json)->optionBuffer
  })->toEqual(Some("0x0235b618ab0f0e9f48b1af32f56b78d955c432279893714737a937035024b83c58"->JsBuffer.fromHex)))

  test("optionBuffer None", () => expect(None->optionBuffer)->toEqual(None))

  test("optionTimeS", () => expect({
    let json = `1683785808`->Js.Json.parseExn
  ...
  test("hash", () => expect({
    let json = `"0x1a4ef77bacf1634c08f1ab519c255f2abbf34742f18e91436b3d44190753ac51"`->Js.Json.parseExn

    json->hash
  })->toEqual("1a4ef77bacf1634c08f1ab519c255f2abbf34742f18e91436b3d44190753ac51"->Hash.fromHex))

  test("intToCoin", () => expect({
    12345678->intToCoin
  ...
  test("coinWithDefault", () => expect({
    let json = `"123.00"`->Js.Json.parseExn

    Some(json)->coinWithDefault
  })->toEqual(123.00->Coin.newUBANDFromAmount))

  test("coinWithDefault Default case", () => expect(None->coinWithDefault)->toEqual(0.->Coin.newUBANDFromAmount))
  ...
  test("numberWithDefault", () => expect({
    let json = `123.00`->Js.Json.parseExn

    Some(json)->numberWithDefault
  })->toEqual(123.00))

  test("numberWithDefault not a number", () => expect({
    let json = `"test"`->Js.Json.parseExn
  ...
  test("floatString", () => expect({
    let json = `"123.0"`->Js.Json.parseExn

    json->floatString
  })->toEqual(123.0))

  test("floatExn", () => expect({
    let json = `123.0`->Js.Json.parseExn
