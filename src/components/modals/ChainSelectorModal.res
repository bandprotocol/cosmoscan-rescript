...
let make = (~targetChain) => {
  ...
  <div className=Styles.container>
    <Button
      variant=Button.Outline px=0 py=0 onClick={_ => backModal()} style={Styles.backButton(theme)}>
        ...
        <>
          ...
          ->Belt.Array.keep(transferableChain => {
            ...
            | IBCConnectionQuery.IBC({name, chainID})
              if Js.String.includes(searchTerm, name->Js.String.toLowerCase) ||
              Js.String.includes(searchTerm, chainID->Js.String.toLowerCase) ||
              searchTerm->Js.String.length <= 0 => true
            | _ => false
            }
          })
          ->Belt.Array.map(transferableChain => {
            let keyString = switch transferableChain {
            ...
            let isEqual = {
              ...
              | (IBC({channel: targetChannel, chainID: targetChainID}), IBC({channel, chainID}))
                if targetChannel === channel && targetChainID === chainID => true
              | _ => false
              }
            }
